...
{
    // เริ่มต้นการรัน Seeder สำหรับข้อมูลครุภัณฑ์เริ่มต้น
    public function run(): void
    {
        $now  = Carbon::now();

        // ดึงข้อมูลแผนกโดยใช้ code เป็นคีย์เพื่อนำ ID มาใช้งาน
        $dept = DB::table('departments')->pluck('id','code'); 

        // ดึงข้อมูลหมวดหมู่โดยใช้ slug เป็นคีย์เพื่อนำ ID มาใช้งาน
        $cat  = DB::table('asset_categories')->pluck('id','slug');

        // ตรวจสอบว่ามีข้อมูลพื้นฐานครบถ้วนหรือไม่
        if ($dept->isEmpty() || $cat->isEmpty()) {
            $this->command->warn('⚠️ กรุณา seed departments และ asset_categories ให้เรียบร้อยก่อน');
            return;
        }

        // รายการครุภัณฑ์ตัวอย่างแยกตามหมวดหมู่
        $rows = [
            // เครื่องมือแพทย์
