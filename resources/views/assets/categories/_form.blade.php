...
<div class="maint-form space-y-6">
    ...
    <div class="space-y-1.5">
        ...
        <textarea name="description" rows="2"
            class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm
             focus:border-emerald-500 focus:ring-emerald-500 bg-white">{{ old('description', $category->description) }}</textarea>
        @error('description')
            <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-1.5">
            ...
            <input type="text" name="color" placeholder="#0E2B51" value="{{ old('color', $category->color) }}"
                class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm
               focus:border-emerald-500 focus:ring-emerald-500 bg-white">
            @error('color')
                <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
