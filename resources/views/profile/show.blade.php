...
<div class="w-full flex flex-col min-h-screen bg-white">
            ...
            <div class="flex items-center justify-between w-full">
                ...
                <div class="flex items-center">
                    @if (Route::has('profile.edit'))
                        <a href="{{ route('profile.edit') }}" class="inline-flex items-center gap-2 px-4 py-1.5 bg-white text-slate-700 border border-slate-200 rounded-lg text-[14px] font-medium hover:bg-slate-50 transition-colors">
    ...
    <div class="px-4 md:px-6 lg:px-8 py-10 max-w-5xl mx-auto w-full">
        ...
        <div class="flex items-center gap-6 mb-12">
            <div class="h-20 w-20 rounded-full overflow-hidden flex items-center justify-center bg-emerald-600 shrink-0 border border-emerald-700/10">
                @if($avatarThumb || $avatarMain)
                    <img src="{{ $avatarThumb ?: $avatarMain }}" alt="{{ $user->name }}" class="h-full w-full object-cover">
                @else
        ...
        <div class="space-y-6">
            ...
            <div class="w-full">
                {{-- ชื่อ-นามสกุล --}}
                <div class="grid grid-cols-1 md:grid-cols-3 py-4 border-b border-slate-100 items-center hover:bg-slate-50/50 transition-colors">
                    <div class="text-[14px] font-bold text-slate-400 uppercase tracking-wider">ชื่อ - นามสกุล</div>
                    <div class="md:col-span-2 text-[15px] font-medium text-slate-800 py-1 md:py-0">{{ $user->name }}</div>
                </div>

                {{-- อีเมล --}}
                <div class="grid grid-cols-1 md:grid-cols-3 py-4 border-b border-slate-100 items-center hover:bg-slate-50/50 transition-colors">
                    <div class="text-[14px] font-bold text-slate-400 uppercase tracking-wider">อีเมลที่ใช้งาน</div>
                    <div class="md:col-span-2 text-[15px] font-medium text-slate-800 py-1 md:py-0">{{ $user->email ?? '-' }}</div>
                </div>

                {{-- เลขบัตรประชาชน --}}
                <div class="grid grid-cols-1 md:grid-cols-3 py-4 border-b border-slate-100 items-center hover:bg-slate-50/50 transition-colors">
                    <div class="text-[14px] font-bold text-slate-400 uppercase tracking-wider">หมายเลขบัตรประชาชน</div>
                    <div class="md:col-span-2 text-[15px] font-medium text-slate-800 py-1 md:py-0 tracking-widest">{{ $user->citizen_id ?? '-' }}</div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 py-4 border-b border-slate-100 items-center hover:bg-slate-50/50 transition-colors">
                    ...
                    <div class="md:col-span-2 text-[15px] font-bold text-emerald-700 py-1 md:py-0">{{ $user->role_label ?? $user->role }}</div>
                </div>

                {{-- วันที่สมัคร --}}
                <div class="grid grid-cols-1 md:grid-cols-3 py-4 border-b border-slate-100 items-center hover:bg-slate-50/50 transition-colors">
                    <div class="text-[14px] font-bold text-slate-400 uppercase tracking-wider">วันที่สมัครสมาชิก</div>
                    <div class="md:col-span-2 text-[14px] text-slate-500 py-1 md:py-0">{{ $user->created_at?->format('d/m/Y H:i') }}</div>
                </div>
            </div>
        </div>
