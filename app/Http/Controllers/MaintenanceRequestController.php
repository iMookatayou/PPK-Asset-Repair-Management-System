...
class MaintenanceRequestController extends Controller
{
    ...
    private function applyStatusTransition(MR $locked, string $toStatus, ?int $actorId, ?string $note = null, string $action = MaintenanceLog::ACTION_TRANSITION): void
    {
        ...
        if ($locked->isFillable('status_updated_by')) $locked->status_updated_by = $actorId;
        if ($locked->isFillable('status_updated_at')) $locked->status_updated_at = $now;

        $locked->save();

        if (!empty($locked->asset_id)) {

            // เมื่อเริ่มซ่อม → asset เป็น in_repair
            if ($toStatus === MR::STATUS_IN_PROGRESS) {
                \App\Models\Asset::whereKey($locked->asset_id)
                    ->where('status', 'active')
                    ->update(['status' => 'in_repair']);

                \Illuminate\Support\Facades\Log::info('[applyStatusTransition] asset set to in_repair', [
            ...
            // เมื่องานจบ/ยกเลิก → asset กลับเป็น active (ถ้าไม่มี MR อื่นค้างอยู่)
            if (in_array($toStatus, [MR::STATUS_RESOLVED, MR::STATUS_CLOSED, MR::STATUS_CANCELLED], true)) {
                ...
                if (!$stillInRepair) {
                    ...
                    \Illuminate\Support\Facades\Log::info('[applyStatusTransition] asset restored to active', [
                        'asset_id'   => $locked->asset_id,
                        'request_id' => $locked->id,
                        'to_status'  => $toStatus,
                        'actor_id'   => $actorId,
                    ...
                    \Illuminate\Support\Facades\Log::info('[applyStatusTransition] asset kept in_repair (other MR still active)', [
                        'asset_id'   => $locked->asset_id,
                        'request_id' => $locked->id,
                    ]);
                }
        ...
        $this->writeTransitionLog($locked, $actorId, $action, $from, $toStatus, $note);
    }
    ...
    public function indexPage(Request $request)
    {
        ...
        // NEW: type filter
        $typeId   = $request->string('type_id')->toString(); // อาจเป็น '', '__null__', หรือ id
    ...
    public function showPage(MR $req)
    {
        ...
        $req->loadMissing([
            ...
            'reporter:id,name,email',
            'technician:id,name',
            // 👇 แก้ไขบรรทัด assignments ดึงเฉพาะคนที่ยังไม่ถูกยกเลิก
            ...
            // 👆 (เอาบรรทัด 'assignments.user:id,name...' แบบเดิมออก แล้วใช้แบบด้านบนแทน)
        ...
        // ดึงข้อมูลประเภทและแคชไว้ 1 ชั่วโมง
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($data, $request, $user, $actorId, $departmentId, &$req) {
            ...
            if ($request->hasFile('files')) {
                foreach ((array) $request->file('files') as $up) {
    ...
    public function update(Request $request, MR $req)
    {
        ...
        $rules = [
            ...
            // 👇👇 [แทรกเพิ่มส่วนที่ 1] เผื่อฟอร์มหน้า Panel ของคุณส่ง array รายชื่อมา จะได้ผ่าน Validation
        ...
        if ($validator->fails()) {
            return redirect()->back()
                ...
                ->withInput()
                ->with('toast', Toast::warning('ข้อมูลไม่ถูกต้อง กรุณาตรวจสอบอีกครั้ง', 2600));
        }
        ...
        DB::transaction(function () use (&$data, $request, $req, $isTeam, $actorId) {
            ...
            // เก็บค่า ID ใหม่ที่ส่งมาจาก Request ไว้เทียบ (สำคัญมาก)
            $incomingTechId = array_key_exists('technician_id', $data)
                ? (int) ($data['technician_id'] ?? 0)
                : $originalTechId;

            // 👇👇 [แทรกเพิ่มส่วนที่ 2] ดักจับการแก้ไขช่างจาก Panel ถ้ามีสัญญาณว่าอัปเดตแต่ข้อมูลว่าง ให้ล้างค่าเป็น 0
            ...
            if (!$isTeam) {
                ...
                unset(
                    $data['technician_id'],
                    // 👇👇 [แทรกเพิ่มส่วนที่ 3] ถอดค่ายกเว้น user_ids ออกด้วยเพื่อความปลอดภัย
                    $data['user_ids'],
                    // 👆👆
                    $data['cost'],
                ...
                // ถ้าไม่ใช่ Team ค่า incoming ต้องเป็นตัวเดิมเสมอ
            ...
            // assignment เช็คการเปลี่ยนแปลงโดยเทียบกับ incomingTechId
            $techChanged   = $isTeam && $originalTechId !== $incomingTechId;
            $statusChanged = array_key_exists('status', $data) && $originalStatus !== $req->status;

            // ถอดเงื่อนไข > 0 ออกเพื่อให้ syncAssignment ทำงานตอนล้างค่าได้
            if ($isTeam && ($techChanged || $statusChanged || $forceUpdateTeam)) {

                $this->syncAssignment($req, $incomingTechId, $actorId, true);

                if (method_exists($this, 'syncAssignments') && $forceUpdateTeam) {
                    $this->syncAssignments($req, $incomingUserIds ?: [], $actorId);
                } elseif (method_exists($this, 'syncAssignments') && $techChanged && !$forceUpdateTeam) {
                    $this->syncAssignments($req, $incomingTechId > 0 ? [$incomingTechId] : [], $actorId);
                }
                // 👆👆
            }
    ...
    public function acknowledgeCase(Request $request, MR $req)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($req, $actorId) {
                // // ใช้ LockForUpdate เพื่อป้องกัน Race Condition กรณีมีคนกดรับทราบพร้อมกัน
                $locked = MR::query()
                    ->whereKey($req->id)
                    ->lockForUpdate()
                    ->firstOrFail();
                ...
                // // บันทึกการเปลี่ยนสถานะผ่าน Helper Method
                ...
                // // Log ภายใน Transaction เพื่อยืนยันว่าการทำงานสำเร็จในระดับ DB
            ...
            return back()->with('toast', Toast::success('รับทราบแล้ว', 1800));
        } catch (\Illuminate\Auth\Access\AuthorizationException $e) {
            // // กรณีไม่มีสิทธิ์ตาม Policy
            return back()->with('toast', Toast::warning('คุณไม่มีสิทธิ์รับทราบรายการนี้', 2200));
        } catch (\Throwable $e) {
            // // Log ข้อผิดพลาดที่เกิดขึ้นพร้อม Context
    ...
    public function acceptCase(Request $request, MR $req)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($req, $actorId) {
                ...
                // Sync ข้อมูลไปยังตาราง Assignment เพื่อระบุตัวตนและหน้าที่ (Lead)
                // แก้ไขตัวแปรให้เป็น array และใส่ s ให้ตรงกับ Method ที่ประกาศไว้
            ...
            return back()->with('toast', Toast::success('รับเรื่องแล้ว', 1800));

        } catch (\Illuminate\Auth\Access\AuthorizationException $e) {
            // กรณี Gate ไม่ให้ผ่าน
            return back()->with('toast', Toast::warning('คุณไม่มีสิทธิ์รับเรื่องรายการนี้', 2200));
    ...
    protected function applyTransition(MR $req, array $data, ?int $actorId = null): MR
    {
        DB::transaction(function () use ($req, $data, $actorId) {
            ...
            // // ลำดับสถานะที่อนุญาต (Logic เดิมของคุณ)
            $allowedNext = [
                'pending'      => ['acknowledged', 'cancelled'],
                'acknowledged' => ['accepted', 'cancelled', 'rejected'],
                'accepted'     => ['in_progress', 'cancelled'],
                'in_progress'  => ['resolved', 'cancelled', 'on_hold'],
            ...
            // // การจัดการเรื่องช่าง (Logic เดิมของคุณ)
            ...
            // // ตั้งค่าช่างอัตโนมัติหากยังไม่มี (Logic เดิมของคุณ)
            ...
            // // จัดการ Timestamp รายสถานะ (Switch Case เดิมของคุณ)
    ...
    protected function respondWithToast(
        ...
        // // ดึงค่ามาตรฐานสำหรับ Toast (ใช้ Helper เพื่อความปลอดภัยของข้อมูล)
        $toastData = [
        ...
        // // กรณีเป็นการเรียกผ่าน API หรือ AJAX (JSON Response)
    ...
    public function edit($id)
    {
        ...
        // // ดึงข้อมูลไฟล์แนบโดยเลือกเฉพาะคอลัมน์ที่จำเป็น (Optimization)
    ...
    {
        // // ดึงชื่อผู้กระทำรายการ (Actor) โดยเลือกเฉพาะคอลัมน์ name เพื่อประหยัด Memory
        $actorName = $actorId
        ...
        return match ($status) {
            ...
            MR::STATUS_ON_HOLD      => 'พักงานชั่วคราว',
    ...
    public function printWorkOrder(Request $request, MR $req)
    {
        ...
        // // ข้อมูลส่วนหัวของเอกสาร (Master Data สำหรับหน้าพิมพ์)
        $hospital = [
        ...
        // // กำหนดชื่อไฟล์ตามเลขที่ใบงาน (Request No.) หรือ ID
        $fileName = sprintf(
            'maintenance-work-order-%s.pdf',
            $req->request_no ?? $req->id
        );

        // // สร้าง PDF จาก View ที่กำหนด
    ...
    protected function resolveSort(Request $request): array
    {
        ...
        // // 1. จัดการการเรียงลำดับคอลัมน์ (Sort By)
        ...
        // // 2. จัดการทิศทางการเรียงลำดับ (Sort Direction)
    ...
    {
        // 1. ถ้าส่งค่าว่างมา (ติ๊กออกหมด) ให้ลบทิ้งทุกคนแบบถาวรเพื่อให้หายจาก Panel
        if (empty($userIds)) {
            MaintenanceAssignment::where('maintenance_request_id', $req->id)->forceDelete();
            Log::info('[MaintenanceRequest] All workers removed completely', ['mr_id' => $req->id]);
            return;
        }

        // 2. ลบคนเก่าที่ไม่ได้ถูกติ๊กเลือกในรอบนี้ออกแบบถาวร
        ...
        // 4. วนลูปบันทึกหรืออัปเดตคนที่มีรายชื่อ
        foreach ($userIds as $index => $userId) {
            $workerRole = User::query()->whereKey($userId)->value('role') ?? 'technician';
            $isLead = ($index === 0);

            // บันทึกเฉพาะคนที่จะทำงานจริงๆ
    ...
    protected function suggestTechUsersForRequest(MR $req): \Illuminate\Support\Collection
    {
        ...
        // // เตรียม Query พื้นฐานสำหรับกลุ่มทีมงาน (Admin, Supervisor, Technician)
        $base = User::query()
        ...
        // // 1) ดึงรายชื่อช่างที่เป็น Default สำหรับงานประเภทนี้ (ลำดับความสำคัญสูงสุด)
    ...
    public function updateType(Request $request, MR $req)
    {
        ...
        // // ตรวจสอบความถูกต้องของข้อมูล (ประเภทใบงานต้องมีอยู่ในระบบ)
